...
if [[ "$HAS_YCKMS_PATCH" != true ]]; then
  CUT_VERSION=$(echo $BASE_VERSION | cut -c 2-)
  ...
  sed -i '' "s/ARG BASE_VAULT_VERSION=.*/ARG BASE_VAULT_VERSION=$CUT_VERSION/" yandex/docker/Dockerfile
  sed -i '' "s/BASE_VAULT_VERSION=.*/BASE_VAULT_VERSION=$CUT_VERSION/" yandex/compute/install.sh
